...
- `rescuedProps` support — a prop whose server-side resolver threw and was rescued (`Inertia::defer(..., rescue: true)`) is reported as "failed to resolve on the server and was rescued" instead of being mistaken for a missing prop. Only newly-rescued props warn; the list persists across visits until a partial reload re-requests them. Available since Inertia 3.4.0, where `rescuedProps` is a required field of the page object — not 3.6 ([@skryukov])
